...
def mmk(s, v, r):
        ...
        dv=np.zeros((len(v),1))
        dv[0] = s*(-delta * Jant + Jhyd + 0.5*Jserca)                  # adpc -- stoichiometry serca: cf. Tran 2009
        dv[1] = s*(Jant - Jf1 - Jsl)                                   # adpm
        dv[2] = s*(Jidh - Jkgdh)                                       # akg
        dv[3] = 0.                                                      # asp; Jaat - Jc_asp
        dv[4] = -dv[0]                                                  # atpc
        dv[5] = -dv[1]                                                  # atpm
        dv[6] = s * uMmM * fc *(-Jserca + Jerout + delta * (Jncx - Juni)) + r*np.sqrt(uMmM * fc *Jip3R)                # cac
...
tnew, ynew = t0, y0
sqrtdt = np.sqrt(dt)
#N = 20.                                    # Number of channels in IP3R clusters
#lmda = 1.                                  # Rescaling factor of the noise
sqrtdtoN = sqrtdt / N

with h5py.File(filename, "a", libver='latest') as output:
	dsett = output.create_dataset('t', ((NTprint, )), chunks=True, dtype='float')
	dsety = output.create_dataset('y', ((18, NTprint)), chunks=True, dtype='float')
